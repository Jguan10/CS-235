...
Date: 10/27/23
Modified from Georgina Woo's Scoundrel.cpp file
...
void Scoundrel::setDagger(const std::string& dagger)
{
    ...
    for(long unsigned int i = 0; i < dagger_upper.length(); i++)
    {
...
bool Scoundrel::setFaction(const std::string& faction)
{
    ...
    for(long unsigned int i = 0; i < faction_upper.length(); i++)
    {
...
void Scoundrel::display() const {
    ...
    std::string disguise = hasDisguise() ? "TRUE\n\n" : "FALSE\n\n";
...
void Scoundrel::eatTaintedStew() {
    ...
    if(getRace() == "UNDEAD") {
        ...
        if(getFaction() == "CUTPURSE") {
            ...
            // if the number is 1 <= x <= 30, then set hp to one and set dagger to "WOOD"
