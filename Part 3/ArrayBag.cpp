...
Modified from the provided ArrayBag.cpp file

Description:
ArrayBag.cpp defines the constructors and private and public function implementation of the ArrayBag class
The ArrayBag class serves an array with member functions to check and modify the array
*/
...
void ArrayBag<ItemType>::operator/= (const ArrayBag<ItemType>& array_bag) {
   ...
   // Loops through the array to check if the array contains the item
   for(int i = 0; i < array_bag.getCurrentSize(); i++) {

      // If the item doesn't already exist, add it to the array
      if(getFrequencyOf(array_bag.items_[i]) < 1)
