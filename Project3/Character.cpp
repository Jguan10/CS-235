...
void Character::setRace(const std::string& race) {
    ...
    else if(race_upper== "ELF")
    {
        race_ = ELF;
    }
    else if(race_upper == "DWARF")
    {
        race_ = DWARF;
    }
    else if(race_upper == "LIZARD")
    {
        race_ = LIZARD;
    }
    else if(race_upper == "UNDEAD")
    {
        race_ = UNDEAD;
    }
    else
...
bool Character::operator== (const Character& character) const{
    ...
    // Checks to see if all the private members in character1 == character2's private members
    return (character.getName() == getName() && character.getRace() == getRace() && character.getLevel() == getLevel() && character.isEnemy() == isEnemy());
...
void Character::display() {
    ...
    // Checks boolean enemy to output "They are" if true and "They are not" if false
    std::string enemy_status = enemy_ ? "They are" : "They are not";
   
    std::cout << name_ << " is a Level " << level_ << " " << race_ << ".\n"
    << "Vitality: " << vitality_ << "\n" << "Max Armor: " << armor_ << "\n"
    << enemy_status << " an enemy. \n";
