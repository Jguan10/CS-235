...
Date: 10/27/23
Modified from Georgina Woo's Ranger.cpp file
...
bool Ranger::addArrows(const std::string& type, const int& quantity)
{
    ...
    for(long unsigned int i = 0; i<type_upper.size(); i++)
    {
...
bool Ranger::addAffinity(const std::string& affinity)
{
    ...
    for(long unsigned int i = 0; i<aff_upper.size();i++)
    {
...
void Ranger::display() const {
    ...
    // Strings for printing enemy and companion status
    std::string enemy = isEnemy() ? "They are an enemy." : "They are not an enemy.";
    std::string companion = getCompanion() ? "TRUE" : "FALSE";

    std::cout << getName() << " is a Level " << getLevel() << " " << getRace() << " RANGER."
        << "\nVitality: " << getVitality() << "\nArmor: " << getArmor() << "\n" << enemy
        << "\nAnimal Companion: " << companion;

    // Printing out the arrows
    std::cout << "\nArrows: ";
    for(long unsigned int i = 0; i < arrows_.size(); i++) {
        std::cout << "\n" <<arrows_[i].type_ << ": " << arrows_[i].quantity_;
    }

    // Printing out affinities
    std::cout << "\nAffinities: ";
...
/*
    @param  : A string representing a possible affinity
    @post   : Loop through the affinity vector and return true if the affinity provided is there, false if it isn't
*/
bool Ranger::hasAffinity(std::string affinity) {
    
    std::vector <std::string> affinities = getAffinities();
    
    for(long unsigned int i = 0; i < affinities.size(); i++) {
        if(affinities[i] == affinity)
            return true;
    }
    return false;
}
